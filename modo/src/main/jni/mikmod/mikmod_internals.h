...
extern void mikmod_android_setBoostLevel(int);
extern void mikmod_android_getSamples(SBYTE*);
extern short* mikmod_android_getSamples2();
extern void mikmod_android_howmany(int); 
...
MREADER *_mm_new_mem_reader(const void*, int);
void _mm_delete_mem_reader(MREADER*);
...
enum {
	...
	UNI_MEDEFFECTFD,   /* set note pitch */	
	UNI_MEDEFFECT1F,   /* delay than retrigger note */
	/* Oktalyzer effects */
	UNI_OKTARP,		   /* arpeggio */
...
typedef struct MP_CONTROL {
	...
	UBYTE		volslide_mem; /* pteffect5, vol slide memory */
} MP_CONTROL;
