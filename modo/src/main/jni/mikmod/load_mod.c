...
static UBYTE ConvertNote(MODNOTE *n, UBYTE lasteffect)
{
	...
	instrument = (n->a & 0x10) | (n->c >> 4);     // WB: SampleNr
	period = (((UWORD)n->a & 0xf) << 8) + n->b;   // aka playback freq
	effect = n->c & 0xf;  // effect nr
	effdat = n->d;  // effect parameter
	...
	if (instrument) {
		...
		if ((instrument > 31) || (!mh->samples[instrument - 1].length)) {
			UniPTEffect(0xc, 0); // WB: Set Volume to zero
			if (effect == 0xc)
				effect = effdat = 0;
		} else {
			/* Protracker handling */
...
static UBYTE *ConvertTrack(MODNOTE *n, int numchn)
{
	...
	for (t = 0; t < 64; t++) {
		...
		UniNewline();
		n += numchn;  // WB: modnotes sind sequenziell ablegt, also die notes für die anderen chan überspringen
	}
	return UniDup();
}
...
static BOOL ML_LoadPatterns(void)
{
	...
	if (trekker && of.numchn == 8) {
		...
		for (t = 0; t < of.numpat; t++) {
			...
			// 64 lines pro pattern, 1 line = 4 bytes (modnote) * numchan
			
			// modnotes[0] = erste modnote
			// modnotes[1] = zweite modnote
			// modnotes[2] = dritte modnote
			...
			// WB: Eine Track ist ist das komplette Pattern in UNI format konvertiert,
			// Index = patNr * numchn + chn > unipattern für dieses pattern und diesen channel
...
static BOOL MOD_Load(BOOL curious)
{
	...
	for (t = 0; t < of.numins; t++) {
		...
		q->speed = finetune[s->finetune & 0xf];  // WB: snibble. use only lower 4 bitS!
		q->volume = s->volume & 0x7f;  // WB: max 64, use only lower 7 bits!
		q->loopstart = (ULONG)s->reppos << 1;  // WB: mod stores length as words, so *2 offset in bytes
		q->loopend = q->loopstart + ((ULONG)s->replen << 1);  // WB: Same here!
		q->length = (ULONG)s->length << 1; // WB: ..AND AGAIN
		q->flags = SF_SIGNED;  // WB: mod uses 8bit signed PCM
