...
static UBYTE* IT_ConvertTrack(ITNOTE* tr,UWORD numrows)
{
	...
	for(t=0;t<numrows;t++) {
		...
		if (tr[t*of.numchn].cmd == 0x13 && ((tr[t*of.numchn].inf)>>4) == 0x07 && memcmp(mh->songname, "Anatomy", 7) == 0) {
			// Ugly hack for Anatomy.it, the volume envelope off (S77) does not work as intended
		} else {
...
BOOL IT_Load(BOOL curious)
{
	...
	// wb hack - some IT/S3 file has ordnum>256, but poslookupcnt is only one byte
	if(mh->ordnum > 255)
		poslookupcnt=255;
	//
	...
	for(t=0;t<mh->smpnum;t++) {
		...
		//s.c5spd = s.c5spd & 0x7ffffL;

		//if(_mm_eof(modreader)||(s.c5spd>0x7ffffL)||(s.length>0xfffffffUL)) {
		if(_mm_eof(modreader)||(s.length>0xfffffffUL)) {
