...
int outofrange;
	...
	for(t=0;t<of.numins;t++,d++) {
		...
		// *** Hacked in to allow some strange xm
		if(ih.numsmp > 16)
			ih.numsmp = 16;

		if (ih.numsmp == 0)	{
			_mm_fseek(modreader,(int)ih.size - 33,SEEK_CUR);
			continue;
		}
		// *** 
...
BOOL XM_Load(BOOL curious)
{
	...
	//mm_read_UBYTES(mh->orders,256,modreader);
...
#ifdef HAVE_SNPRINTF
	snprintf(modtype,60,"%s (XM %d.%02d)",
	                    tracker,mh->version>>8,mh->version&0xff);
#else
	sprintf(modtype,"%s (XM %d.%02d)",
	                tracker,mh->version>>8,mh->version&0xff);
#endif
