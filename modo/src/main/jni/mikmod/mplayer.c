...
{
	if (!tick && a->oldnote && a->main.period) {
...
static int DoMEDEffect1F(UWORD tick, UWORD flags, MP_CONTROL *a, MODULE *mod, SWORD channel)
{
	...
	if (!tick)	
		DoEEffects(tick, flags, a, mod, channel, 0xd0|(dat >> 4));

	if (!a->main.notedelay)
		DoEEffects(tick, flags, a, mod, channel, 0x90|(dat & 0xf));
...
static effect_func effects[UNI_LAST] = {
		...
		DoMEDEffectFD,	/* UNI_MEDEFFECTFD */		
		DoMEDEffect1F,	/* UNI_MEDEFFECT1F */
		DoOktArp,		/* UNI_OKTARP */
};
...
static int fillInTimeOpcodes(MODULE *mod, int patternNr, int row, UBYTE *optable, UBYTE *datatable)
{
	int chn;
	int patternEndReached = 0;
	
	memset(optable, 0, 255);			
	memset(datatable, 0, 255);			
	
	for (chn = 0; chn < mod->numchn; ++chn)
	{
	
//		UBYTE *track = mod->tracks[patternNr * mod->numchn + chn];
		int tr = mod->patterns[patternNr * mod->numchn + chn];
		UBYTE *track = tr < mod->numtrk ? mod->tracks[tr] : NULL;
		UBYTE *currentRow = UniFindRow(track, row);
		...
		UBYTE opcode;
		UniSetRow(currentRow);		
		while ((opcode = UniGetByte()) != 0)
		{
			switch (opcode)
			{
				case UNI_PTEFFECTD:     /* pattern break */
				case UNI_PTEFFECTB:     /* pattern jump */					
				case UNI_PTEFFECTF:     /* set speed */
				case UNI_PTEFFECTE:		/* pattern loop, pattern delay */
				case UNI_S3MEFFECTA:	/* speed set S3M, speed > 128 = speed - 128; */
				case UNI_S3MEFFECTT:	/* set bpm */
					optable[chn] = opcode;
					datatable[chn] = UniGetByte(); // every time related effect has one byte as argument
//					printf("-> effekt: %d -> %d\n", opcode, datatable[opcode]);
					break;
				default:
					UniSkipOpcode();
					break;
	...
	return patternEndReached == mod->numchn;
}

UBYTE opcodes[255];
UBYTE data[255];

ULONG Player_EstimateTime(MODULE *mod)
{
	
	UWORD row = 0;
	SWORD pos = 0;
	UWORD tempo = mod->sngspd;
	UWORD bpm = mod->bpm;
//	UWORD patternEndReached = 1;	
//	float time = 0;
//	float secondsPerLine = tempo * 0.02;
	
	ULONG sngremainder = 0;
	ULONG sngtime = 0;
	...
	void addTicks(int count)
	{
		...
		for (i = 0; i < count; ++i)
		{	
			sngremainder += (1<<9)*5; /* thus 2.5*(1<<10), since fps=0.4xtempo */
			sngtime += sngremainder / bpm;
			sngremainder %= bpm;
		}
	}
	
	// play the song
	while (pos != -1 && pos < mod->numpos)
	{
		int loopStartRow = 0;
		int loop = -1;
		
//		printf("Now looking at Pos %d, Pattern %d, Rows %d\n", pos, mod->positions[pos], mod->pattrows[mod->positions[pos]]);
		int breakPattern = 0;
		
		while (breakPattern == 0 && fillInTimeOpcodes(mod, mod->positions[pos], row, opcodes, data) == 0)
		{
		//	printf("Pos %d / %d, Pattern: %d, Row: %d, BPM: %d, Tempo: %d, Sngtime: %d\n", pos, mod->numpos, mod->positions[pos], row, bpm, tempo, sngtime);
...
//					continue;
				...
				if (opcodes[chn] == UNI_S3MEFFECTT)
				{
					bpm = (data[chn] < 32) ? 32 : data[chn];
					printf("S3_BPM: %d\n", tempo);
//					addTicks(tempo - 1);
//					continue;
				}
			
				if (opcodes[chn] == UNI_PTEFFECTF && data[chn])  // tempo zero is illegal
				{
					if (data[chn] >= mod->bpmlimit)
					{
						printf("BPM: %d\n", data[chn]);
						bpm = data[chn];
//						addTicks(tempo - 1);
					...
					{
						tempo = data[chn] >= mod->bpmlimit ? mod->bpmlimit - 1 : data[chn];
						printf("TEMPO: %d\n", tempo);
//						addTicks(tempo);
					}
//					continue;
				...
				if (opcodes[chn] == UNI_PTEFFECTB && patbrk == 0) // position jump, usually to repeat a song, anyway the current patterns ends after this
				{
					if (data[chn] <= pos || data[chn] >= mod->numpos)  // avoid endless loop
					{
						printf("JUMP POS, out of range: %d\n", data[chn]);
						pos = mod->numpos + 1;
					} else
					{
						printf("JUMP POS, within range: %d\n", data[chn]);
						pos = data[chn];
						row = 0;
					}
					breakPattern = 1;
...
//					break;
				...
				if (opcodes[chn] == UNI_PTEFFECTD)  // pattern break, move to next pattern @ row
				{			
					printf("PATTERNBREAK: %d\n", data[chn]);
					pos += 1;
					row = data[chn];
					patbrk = 1;
					breakPattern = 1;
//					addTicks(tempo - 1);
				...
				if (opcodes[chn] == UNI_PTEFFECTE)
				{
					UBYTE nib = data[chn] & 0xf;
					switch (data[chn] >> 4)
					{
						case 0x06:
							if (looping == 0)
...
//						addTicks(tempo - 1);
						break;
					case 0x0E:
						printf("PATTERN DELAY: %d\n", data[chn]);
						addTicks(tempo * nib - tempo);
						break;
					}
				}
			}			
...
//		printf("--> Pattern %d ended @ row: %d / %d, DIFF: %d\n", mod->positions[pos], row, mod->pattrows[mod->positions[pos]], (sngtime - old));
		
		// Has the pattern ended normally? no breaks, jumps? go to next position
		if (breakPattern == 0)
		{
			row = 0;
			pos += 1;
