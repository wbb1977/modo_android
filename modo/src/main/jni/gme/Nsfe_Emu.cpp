...
blargg_err_t Nsfe_Info::track_info_( track_info_t* out, int track ) const
{
	...
	// wbb - more details which expansion chips are used
	if ( info.chip_flags == 0) {
		Music_Emu::copy_field_( out->system, "Famicom (pure APU)" );
	} else
	{
		memset( out->system, 0, 256 );
		strcat( out->system, "Famicom - " );
		if ( info.chip_flags & nsf_header_t::vrc6_mask )
			strcat( out->system, "VRC6 " );
		if ( info.chip_flags & nsf_header_t::vrc7_mask )
			strcat( out->system, "VRC7 " );
		if ( info.chip_flags & nsf_header_t::fds_mask )
			strcat( out->system, "FDS " );
		if ( info.chip_flags & nsf_header_t::mmc5_mask )
			strcat( out->system, "MMC5 " );
		if ( info.chip_flags & nsf_header_t::namco_mask )
			strcat( out->system, "Namco " );
		if ( info.chip_flags & nsf_header_t::fme7_mask )
			strcat( out->system, "FME7 " );
	}
	//
