...
void Ay_Emu::set_voice( int i, Blip_Buffer* center, Blip_Buffer* left, Blip_Buffer* right)
{
	...
	else
	{
		...
		if (i == 0)
			core.apu().set_output(i, left);  // Channel A
		else if (i == 1)
			core.apu().set_output(i, center); // Channel B
		else if (i == 2)
			core.apu().set_output(i, right);  // Channel C
		else
			core.apu().set_output(i, center); // just to make sure
	}
/*
	if ( i >= Ay_Apu::osc_count )
		core.set_beeper_output( center );
