...
static void copy_nsf_fields( Nsf_Emu::header_t const& h, track_info_t* out )
{
	...
	} else
	{
		...
		if ( h.chip_flags & nsf_header_t::vrc6_mask )
			strcat( out->system, "VRC6 " );
		if ( h.chip_flags & nsf_header_t::vrc7_mask )
			strcat( out->system, "VRC7 " );
		if ( h.chip_flags & nsf_header_t::fds_mask )
			strcat( out->system, "FDS " );
		if ( h.chip_flags & nsf_header_t::mmc5_mask )
			strcat( out->system, "MMC5 " );
		if ( h.chip_flags & nsf_header_t::namco_mask )
			strcat( out->system, "Namco " );
		if ( h.chip_flags & nsf_header_t::fme7_mask )
