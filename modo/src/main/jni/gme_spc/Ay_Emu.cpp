...
blargg_err_t Ay_Emu::start_track_( int track )
{
	...
	// and leave the interrupt alone. Took me one day to figure out (TechnicanTed.ay).
	if (mem.ram[0x38] == 0xC9)
...
void Ay_Emu::cpu_out_misc( cpu_time_t time, unsigned addr, int data )
{
	if ( !cpc_mode )
	{
		...
		case 0xAFFD:
			spectrum_mode = true;
			active_apu->write( time, apu_addr, data );
			return;
		case 0xFEFD:
			spectrum_mode = true;
			apu_addr = data & 0x0F;
...
int ay_cpu_in( Ay_Cpu* cpu, unsigned addr )
{
	...
	if (addr == 0xFFFD)
		return emu.active_apu->read(emu.apu_addr);
